...
{ 
  ...
  //G4cout << "First had interaction " <<  this->fEventAction->firstHadInteractionDepth << G4endl;
  
  // getting leakage particles 
  
  G4Track *aTrack = aStep->GetTrack() ;
  //G4double eKin = aTrack->GetKineticEnergy();
  
  
  //if((preStepPoint->GetStepStatus() == fGeomBoundary || postStepPoint->GetStepStatus() == fGeomBoundary) && postStepPoint->GetMaterial() == defaultMaterial){
  //if(postStepPoint->GetMaterial() == defaultMaterial){
  G4StepStatus status = aTrack->GetStep()->GetPostStepPoint()->GetStepStatus();
  if (status == fWorldBoundary) { 
  	this->fEventAction->leakageEnergy += aTrack->GetKineticEnergy(); //eKin;
  }
  //G4cout << "Kinetic Energy " <<  this->fEventAction->leakageEnergy << G4endl;
